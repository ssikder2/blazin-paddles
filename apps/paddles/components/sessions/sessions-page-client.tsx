...
function bookingDisplayTime(booking: CourtBooking): { label: string; range: string } {
  const start = parseISO(booking.start);
  const end = parseISO(booking.end);
  return {
...
export function SessionsPageClient() {
  ...
  const upcoming = sortBookingsUpcoming(bookings).filter((b) => parseISO(b.end) > new Date());
  const budget = 10;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      <h1 className="mb-6 font-semibold text-2xl tracking-tight">My sessions</h1>
      ...
      <section className="mb-10">
        ...
        {upcoming.length === 0 ? (
          <p className="text-[var(--color-muted-foreground)] text-sm">No upcoming sessions.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="text-[var(--color-muted-foreground)] text-sm">{VENUE}</p>
